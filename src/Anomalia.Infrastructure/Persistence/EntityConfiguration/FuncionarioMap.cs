...
internal class FuncionarioMap : IEntityTypeConfiguration<Funcionario>
{
    public void Configure(EntityTypeBuilder<Funcionario> builder)
    {
        ...
        builder.Property(c => c.GestorSetorId)
               .ValueGeneratedNever()
               .HasColumnName("GestorSetorId")
               .HasMaxLength(100);


        builder.HasOne(c => c.Cargo).WithMany().OnDelete(DeleteBehavior.NoAction);
        ...
        builder.HasOne(c => c.Setor).WithMany().HasForeignKey(x=>x.SetorId).OnDelete(DeleteBehavior.NoAction);
